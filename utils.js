...
exports.setNotificationState = (setState, amount, reset) => {
  setState(current => {
    ...
    if (!options.tabBarBadge || reset) options.tabBarBadge = 0;

    options.tabBarBadge += amount;

    //if no notifcations, delete it
    if (options.tabBarBadge <= 0) delete options.tabBarBadge;
...
exports.updateUserMessages = async (setMessages, setNotifications, userId) => {
  ...
  const allNotifications = toSet.reduce(
    (sum, m) => (m.unread > 0 ? ++sum : sum),
    0,
  );
